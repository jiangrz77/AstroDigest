...
class EmailSettingsTests(unittest.TestCase):
    ...
    def test_settings_page_contains_email_handlers(self):
        ...
        self.assertIn("<title>Astro Digest - Settings</title>", page)
        self.assertNotIn("AstroPaperDigest", page)
        self.assertIn('id="enable_email"', page)
        self.assertIn('id="email_address"', page)
        self.assertIn("Sender Email", page)
        self.assertIn('id="email_recipients"', page)
    ...
    def test_deep_link_date_is_validated(self):
        for scheme in ("astrodigest", "astropaperdigest"):
            with self.subTest(scheme=scheme):
                self.assertEqual(
                    _deep_link_date(f"{scheme}://digest/2026-08-23"),
                    "2026-08-23",
                )
                self.assertEqual(_deep_link_date(f"{scheme}://digest/not-a-date"), "")
                self.assertEqual(_deep_link_date(f"{scheme}://digest/2026-02-30"), "")
                self.assertEqual(_deep_link_date(f"{scheme}://other/2026-08-23"), "")
        self.assertEqual(_deep_link_date("https://digest/2026-08-23"), "")
    ...
    def test_send_current_digest_forces_explicit_resend(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with open(digest_path, "w", encoding="utf-8") as handle:
                handle.write("# Astro Digest - 2026-08-23\n")
