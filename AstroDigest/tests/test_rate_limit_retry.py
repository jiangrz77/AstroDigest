...
class RateLimitRetryTests(unittest.TestCase):
    ...
    def test_parse_retry_accepts_seconds_and_http_dates(self):
        class _Resp:
        ...
        with patch('src.fetch_arxiv.time.time', return_value=datetime(
                2026, 10, 21, 7, 26, tzinfo=timezone.utc).timestamp()):
            self.assertEqual(_parse_retry_after(_Resp("Wed, 21 Oct 2026 07:28:00 GMT")), 120.0)
        for invalid in ['not a date', 'NaN', 'inf', '-inf']:
            self.assertIsNone(_parse_retry_after(_Resp(invalid)))

    def test_server_cooldown_is_never_shortened_to_fit_budget(self):
        client = _ScriptedClient([_http_429()])
        for requested, spent in [(600, 0), (200, 300)]:
            with self.subTest(requested=requested, spent=spent):
                client._last_429_retry_after = requested
                budget = _RateLimitBudget()
                budget.total_wait = spent
                self.assertIsNone(budget.next_wait(client))
                self.assertEqual(budget.total_wait, spent)
                self.assertEqual(budget.waits, 0)

    def test_long_cooldown_aborts_without_an_early_network_retry(self):
        client = _ScriptedClient([_http_429()])
        client._last_429_retry_after = 600
        sleep = _run_fetch_until_error(client)
        self.assertEqual(client.calls, 1)
        sleep.assert_not_called()

    def test_client_clears_old_retry_after_on_success(self):
        from src.fetch_arxiv import _new_client
        responses = [Mock(status_code=429, headers={'Retry-After': '120'}),
        ...
        with patch('requests.Session.get', side_effect=responses):
            ...
            self.assertEqual(client._last_429_retry_after, 120)
            client._session.get('https://export.arxiv.org/api/query')
            self.assertIsNone(client._last_429_retry_after)
            client._session.get('https://export.arxiv.org/api/query')
            self.assertIsNone(client._last_429_retry_after)

    def test_tls_failure_discards_pool_without_weakening_verification(self):
        from src.fetch_arxiv import _new_client
        response = Mock(status_code=200, headers={})
        with patch('requests.Session.get', side_effect=[
                requests.exceptions.SSLError('EOF occurred in violation of protocol'), response]), \
             patch('requests.Session.close') as close:
            client = _new_client()
            with self.assertRaises(requests.exceptions.SSLError):
                client._session.get('https://export.arxiv.org/api/query')
            close.assert_called_once()
            self.assertTrue(client._session.verify)
            self.assertTrue(client._session.trust_env)
            self.assertIs(client._session.get('https://export.arxiv.org/api/query'), response)

    def test_tls_then_429_then_success_preserves_separate_retry_budgets(self):
        client = _ScriptedClient([
