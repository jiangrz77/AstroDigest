...
from src.reason_highlight import reason_highlighter
...
class EmailMathTests(unittest.TestCase):
    ...
    def test_reason_emphasis_preserves_math_and_caps_hits_across_prose_chunks(self):
        rendered = render_math_html(
            r"Dark matter <stars> $\mathrm{redshift}$ redshift $z$ pulsar and quasar.",
            render_text=reason_highlighter(["dark matter", "redshift", "pulsar", "quasar"]),
        )

        self.assertIn("<b>Dark matter</b> &lt;stars&gt;", rendered)
        self.assertIn("<b>redshift</b>", rendered)
        self.assertIn("<b>pulsar</b> and quasar.", rendered)
        self.assertEqual(rendered.count("<b>"), 3)
        self.assertEqual(rendered.count("<math "), 2)
        self.assertNotIn(r"\mathrm", rendered)
