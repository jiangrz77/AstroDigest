...
def test_send_digest_email_subject_and_passthrough():
    ...
    assert ok is True
    assert captured["subject"] == "Astro Digest - 2026-08-18"
...
def test_send_digest_email_defaults_to_today():
    ...
    assert captured["subject"] == f"Astro Digest - {date.today().isoformat()}"
    print("  PASSED")


def test_daily_notification_contains_full_four_and_five_star_abstracts_and_thread_headers():
    print("=== Test: daily notification includes full 4/5-star abstracts ===")
    captured = {}
    papers = [
        {
            "id": "2608.12345",
        ...
        },
        {
            ...
            "abstract": "The full four-star overview is included in the daily email.",
        },
    ]
    ...
    with tempfile.TemporaryDirectory() as tmp, mock.patch(
        ...
        "src.notifier.reason_keywords", return_value=["research profile"]
    ):
        state_path = os.path.join(tmp, "email-state.json")
        ok = send_digest_notification(papers, cfg, "2026-08-23", state_path=state_path)

    assert ok is True
    assert captured["subject"] == "Astro Digest | 2026-08-23 Sun | 2 papers"
    assert "complete abstract, including details" in captured["body"]
    assert "A Four Star Paper" in captured["body"]
    for body in (captured["body"], captured["html_body"]):
        assert papers[1]["abstract"] in body
        assert papers[1]["reason"] in body
        assert "5-Star Recommendations" in body
        assert "4-Star Recommendations" in body
    assert "complete abstract, including details" in captured["html_body"]
    assert "Astro Digest Daily Digest" in captured["body"]
    assert "Why recommended:" not in captured["body"]
    assert papers[0]["reason"] in captured["body"]
    assert "There are no 5-star recommendations today." not in captured["body"]
    assert "astrodigest://digest/2026-08-23" in captured["body"]
    assert "http://127.0.0.1" not in captured["body"]
    assert "今日" not in captured["body"]
    assert "五星" not in captured["body"]
    assert "Astro Digest Daily Digest" in captured["html_body"]
    assert "Why recommended:" not in captured["html_body"]
    assert "Directly relevant to the <b style='color:#b45309;font-weight:600'>research profile</b>." in captured["html_body"]
    assert ">Open Astro Digest</a>" in captured["html_body"]
    assert "Open Astro Digest to view the complete digest" not in captured["html_body"]
    assert "astrodigest://digest/2026-08-23" in captured["html_body"]
    assert "http://127.0.0.1" not in captured["html_body"]
    assert captured["headers"]["X-AstroDigest-Date"] == "2026-08-23"
    assert captured["headers"]["Message-ID"]
    if "References" in captured["headers"]:
        assert captured["headers"]["References"]
    assert app_deep_link("2026-08-23") == "astrodigest://digest/2026-08-23"
    print("  PASSED (4/5-star full abstracts + date subject + deep link verified)")
...
def test_daily_notification_deduplicates_and_force_resends_in_same_thread():
    ...
    with tempfile.TemporaryDirectory() as tmp, mock.patch(
        ...
        # Updated content must still respect daily deduplication until the user
        # explicitly requests a resend.
        papers[0]["title"] = "Updated Paper"
        papers[0]["score"] = 4
        assert send_digest_notification(papers, BASE_CONFIG, "2026-08-23", state_path=state_path)
        assert send_digest_notification(papers, BASE_CONFIG, "2026-08-23", force=True, state_path=state_path)

    assert len(calls) == 2
    assert "Updated Paper" in calls[1][1]["html_body"]
    assert "4-Star Recommendations" in calls[1][1]["html_body"]
...
def _live_send(recipient_override=None):
    ...
    body = (
        "This is a test email from the Astro Digest notification feature.\n"
        "If you received it, your SMTP configuration and email sending work correctly.\n\n"
        "This is a test email for the Astro Digest email reminder.\n"
...
TESTS = [
    ...
    test_daily_notification_contains_full_four_and_five_star_abstracts_and_thread_headers,
...
def main():
    ...
    print("=" * 60)
    print("Astro Digest - Email Notifier Tests (mocked, offline)")
    print("=" * 60)
    passed = failed = 0
    for test in TESTS:
        try:
