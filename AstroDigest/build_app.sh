#!/bin/bash
# Build a native, self-contained Astro Digest.app from source.
# Usage: ./build_app.sh
...
APP_DIR="$APP_ROOT/../$APP_DISPLAY_BUNDLE"
...
echo "Building native $APP_DISPLAY_BUNDLE v$VERSION..."
...
"$PYTHON_BIN" -m PyInstaller \
    ...
    --add-binary "$CLI_DIST/astrodigest-cli:." \
    --distpath "$APP_DIST" \
    --workpath "$APP_WORK" \
    --specpath "$BUILD_ROOT" \
    "$APP_ROOT/src/gui.py"
...
PLIST="$APP_DIST/$APP_BINARY_BUNDLE/Contents/Info.plist"
...
codesign --force --deep -s - "$APP_DIST/$APP_BINARY_BUNDLE"
# Keep the executable technical name while giving Finder and the Applications
# folder the product name users see.
mv "$APP_DIST/$APP_BINARY_BUNDLE" "$APP_DIST/$APP_DISPLAY_BUNDLE"
"$PYTHON_BIN" "$APP_ROOT/verify_bundle.py" "$APP_DIST/$APP_DISPLAY_BUNDLE"
cp -R "$APP_DIST/$APP_DISPLAY_BUNDLE" "$APP_DIR"
