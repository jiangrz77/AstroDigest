...
def _mathml_html(tex: str, display: bool) -> str:
    ...
    # Let the MathML display attribute select native inline/block math layout.
    # CSS display:inline/block replaces the inner math formatting context and
    # can put each formula on a separate line in otherwise inline prose.
    # https://www.w3.org/TR/mathml-core/#the-display-property
    style = (
        "text-align:center;margin:0.65em 0;font-size:1em;"
        if display
        else "font-size:1em;vertical-align:-0.08em;"
    )
    attributes = (
...
def render_math_html(
    text: str | None, *, render_text: Callable[[str], str] = html.escape,
) -> str:
    """Render prose safely and replace intact TeX groups with native MathML.

    An optional trusted renderer can emphasize prose without splitting math.
    """
    value = "" if text is None else str(text)
    result: list[str] = []
    cursor = 0
    for match in _MATH_DELIMITER_RE.finditer(value):
        result.append(render_text(value[cursor:match.start()]))
        ...
        cursor = match.end()
    result.append(render_text(value[cursor:]))
    return "".join(result)
