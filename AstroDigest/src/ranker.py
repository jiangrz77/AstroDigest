...
def rank_papers(
    ...
    Returns:
        ...
        'score_adjustment', 'reason', 'technical_terms' and
        'learning_candidates' fields, sorted by score descending
    """
    if not papers:
        return []
    ...
    known_terms = list((profile.get("keywords") or {}).keys())
    known_terms.extend(
        record.get("term") or record.get("canonical_term")
        for record in (profile.get("technical_terms") or [])
        if isinstance(record, dict)
    )
    known_terms.extend((learned_profile.get('keyword_weights') or {}).keys())
    ...
    for idx, paper in enumerate(papers):
        p = paper.copy()
        terms = feedback_technical_terms(p, known_terms=known_terms)
        p['technical_terms'] = terms
        p['learning_candidates'] = [r for r in terms if r['learning_candidate']]
