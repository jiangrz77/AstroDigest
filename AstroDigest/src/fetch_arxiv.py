...
_ARXIV_USER_AGENT = "AstroDigest/2.3.3"
...
def _parse_retry_after(response) -> Optional[float]:
    """Parse Retry-After seconds or an HTTP date; reject non-finite values."""
...
class _RateLimitBudget:
    ...
    def next_wait(self, client) -> Optional[float]:
        ...
        (see ``_new_client``). If the requested delay exceeds our bounded
        wait budget, stop instead of retrying before the server permits it.
        """
        ...
        desired = max(scheduled, retry_after)
        remaining = _RATE_LIMIT_TOTAL_BUDGET - self.total_wait
        if desired > min(_RATE_LIMIT_BACKOFF_CAP, remaining):
            print(f"  arXiv requests a {desired:.0f}s cooldown; this exceeds the remaining retry budget. No early retry will be sent.")
            return None
        wait = desired
        self.waits += 1
...
def _new_client(trust_env: bool = True) -> arxiv.Client:
    ...
    def get_with_timeout(url, **kwargs):
        ...
        try:
            ...
            # Discard pooled transports after an abrupt TLS failure. Keep
            # proxy settings and certificate verification unchanged.
            client._session.close()
            raise
        client._last_429_retry_after = (
...
def _fetch_listed_papers(
    ...
    for start in range(0, len(ids), _ID_LIST_BATCH_SIZE):
        ...
        while True:
            try:
                # Search.max_results limits yielded results, but the arxiv
                # client puts its own page_size into the actual HTTP request.
                client.page_size = len(batch)
                ...
                if connection_attempts == 0:
                    ...
                    if isinstance(e, requests.exceptions.SSLError):
                        print("  arXiv API TLS connection failed; retrying once with a fresh connection...")
                    else:
                ...
                if e.status == 429:
                    ...
                    if wait is not None:
                        ...
                        time.sleep(wait)
                        continue
                    print("  arXiv API rate limited; retry budget exhausted or requested cooldown too long.")
                raise
