...
from .technical_terms import parse_technical_terms_response, TechnicalTermValidationError
...
def parse_digest(digest_path: str) -> dict:
    ...
    # Extract header info
    date_match = re.search(r"# (?:Astro Digest|AstroDigest|AstroPaperDigest|Arxiv Daily Digest) - (\d{4}-\d{2}-\d{2})", content)
    if date_match:
...
def _parse_single_paper(block: str) -> dict:
    ...
    # Strip metadata before interpreting prose: a term reason must not be
    # mistaken for a score, author line, or paper attribute.
    metadata_prefixes = ('<!-- astrodigest-technical-terms:', '<!-- apd-technical-terms:')
    metadata = [line for line in block.splitlines() if line.startswith(metadata_prefixes)]
    ...
    if metadata:
        paper['technical_terms'] = []
        try:
            match = re.fullmatch(r'<!-- (?:astrodigest|apd)-technical-terms:v1 (.+) -->', metadata[0])
            if len(metadata) != 1 or not match:
                raise TechnicalTermValidationError('Invalid or unsupported term metadata')
            paper['technical_terms'] = parse_technical_terms_response(match.group(1))['terms']
        except TechnicalTermValidationError:
            logging.getLogger(__name__).warning('Invalid term metadata for %s', paper['paper_id'])
        paper['learning_candidates'] = [r for r in paper['technical_terms'] if r['learning_candidate']]
