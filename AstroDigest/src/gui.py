#!/usr/bin/env python3
"""Astro Digest - native desktop window backed by a local Flask server.
...
# ~/Library/Application Support/AstroDigest in the frozen .app).
...
from src.reason_highlight import (
    MAX_REASON_HIGHLIGHTS,
    highlight_reason_text as _highlight_reason_text,
    reason_keywords as _reason_keywords,
)
...
from src.technical_terms import canonical_term_key
...
# Keep the legacy filenames for coordination with an already-running older app.
_ASTRODIGEST_APP_SUPPORT_DIR = _paths.app_support_dir()
_ASTRODIGEST_LOCK_PATH = _ASTRODIGEST_APP_SUPPORT_DIR / "apd.lock"
_ASTRODIGEST_RUN_INFO_PATH = _ASTRODIGEST_APP_SUPPORT_DIR / "apd-run.json"
...
<div id="astrodigest-update-banner" style="display:none;width:100%;box-sizing:border-box;background:#2563eb;color:#fff;padding:10px 32px;font-size:13px;align-items:center;justify-content:center;gap:14px;flex-wrap:wrap;text-align:left">
  ...
  <button id="astrodigest-update-later" style="background:transparent;color:#fff;border:1px solid rgba(255,255,255,.5);border-radius:6px;padding:5px 14px;cursor:pointer">Remind Me Later</button>
</div>
<script>
(function () {
  ...
  function showBanner(s) {
    ...
    if (!bar) return;
    document.getElementById("astrodigest-update-text").textContent =
      "New version v" + s.latest + " available (current v" + s.current + ")";
    bar.style.display = "flex";
    document.getElementById("astrodigest-update-now").addEventListener("click", function () {
    ...
    document.getElementById("astrodigest-update-later").addEventListener("click", function () {
...
def _update_config() -> dict:
    ...
    try:
        ...
        with open(os.path.join(_PROJECT_DIR, "config.yaml"), "r", encoding="utf-8") as f:
            cfg = yaml.safe_load(f) or {}
        update_cfg = dict(cfg.get("update", {}) or {})
        # Follow the renamed official repository for configurations saved by
        # previous releases while preserving users' custom update sources.
        if update_cfg.get("github_repo") == "jiangrz77/AstroPaperDigest":
            update_cfg["github_repo"] = "jiangrz77/AstroDigest"
        return update_cfg
    except Exception:
        return {}
...
def _start_update_check():
    ...
    repo = _update_config().get("github_repo", "jiangrz77/AstroDigest")
    with _update_lock:
...
def _download_update(url: str, version: str, expected_sha: str):
    ...
    dest = updater.UPDATES_DIR / f"AstroDigest-v{version}.zip"
...
def _cleanup_run_files():
    ...
    global _run_lock_fh
    try:
        if _ASTRODIGEST_RUN_INFO_PATH.exists():
            _ASTRODIGEST_RUN_INFO_PATH.unlink()
    except OSError:
        pass
...
def _ensure_run_dir():
    try:
        _ASTRODIGEST_APP_SUPPORT_DIR.mkdir(parents=True, exist_ok=True)
    except OSError:
        pass
...
def _read_run_info():
    try:
        with open(_ASTRODIGEST_RUN_INFO_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
...
def _write_run_info(port: int):
    ...
    tmp_path = _ASTRODIGEST_RUN_INFO_PATH.with_suffix(".json.tmp")
    with open(tmp_path, "w", encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False)
    os.replace(tmp_path, _ASTRODIGEST_RUN_INFO_PATH)
...
def _acquire_single_instance_lock() -> bool:
    ...
    global _run_lock_fh
    _ensure_run_dir()
    fh = open(_ASTRODIGEST_LOCK_PATH, "a+")
    try:
...
def _deep_link_date(value: str) -> str:
    """Read current and legacy app links so older digest emails remain usable."""
    try:
        parsed = urlparse(str(value).strip())
        if parsed.scheme not in {"astrodigest", "astropaperdigest"} or parsed.netloc != "digest":
            return ""
...
def _stream_pipeline(cmd: list[str], timeout: int = 900) -> tuple[int, str, str]:
    ...
    staging_dir = tempfile.mkdtemp(prefix="astrodigest-run-", dir=str(staging_parent))
    environment["ASTRODIGEST_OUTPUT_STAGING_DIR"] = staging_dir
...
def run_pipeline(include_cross: bool = True, include_replacements: bool = True, target_date: str = ""):
    ...
    try:
        ...
        if getattr(sys, "frozen", False):
            cmd = [str(Path(sys._MEIPASS) / "astrodigest-cli")]
        else:
        ...
        if _pipeline_cancel_requested:
            ...
            if api_key_failure:
                ...
                _pipeline_message = (
                    ...
                    "Astro Digest retried with increasing backoff waits, but "
...
.astrodigest-cal{position:fixed;z-index:1000;background:#fff;color:#333;border-radius:12px;box-shadow:0 10px 34px rgba(0,0,0,.28),0 2px 8px rgba(0,0,0,.14);padding:14px;width:288px;font-size:13px;user-select:none}
.astrodigest-cal[hidden]{display:none}
...
<script>
(function () {
  var STATUS = window.ASTRODIGEST_DIGEST_STATUS || {};
  ...
  function build() {
    ...
    pop.className = 'astrodigest-cal';
    pop.id = 'astrodigest-cal';
    pop.hidden = true;
    pop.innerHTML =
      '<div class="cal-head">' +
...
<title>Astro Digest - Setup</title>
<style>
...
<form method="POST" action="/setup" enctype="multipart/form-data">
  ...
  <div class="step">
    ...
    <p class="hint">After each successful daily update, receive 4–5 star recommendations with full abstracts, plus a compact overview of all other papers.</p>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Astro Digest - Settings</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f0f2f5;color:#333}
...
<div class="layout">
  ...
  <main class="content">
    ...
    <section class="panel" id="panel-update">
      ...
      <div class="card">
        <h2>About</h2>
        <p class="sub">Astro Digest v{{ current_version }} — LLM-scored daily arXiv digests for your research interests.</p>
        <a href="https://github.com/jiangrz77/AstroDigest" target="_blank" style="font-size:13px;color:#2563eb">GitHub Repository ↗</a>
      </div>
    ...
    <section class="panel" id="panel-interests">
        ...
        <div class="card">
          ...
          <p class="sub">Choose how Astro Digest builds your relevance profile.</p>
          <div class="toggle-group">
            <label><input type="radio" name="profile_mode" value="quick" {% if cur_profile_source == 'quick' %}checked{% endif %} onchange="toggleProfileMode()"> Quick Start</label>
            <label><input type="radio" name="profile_mode" value="bib" {% if cur_profile_source == 'bib' %}checked{% endif %} onchange="toggleProfileMode()"> Use Bib File</label>
            <label><input type="radio" name="profile_mode" value="zotero" {% if cur_profile_source == 'zotero' %}checked{% endif %} onchange="toggleProfileMode()"> Use Zotero Library</label>
    ...
    <section class="panel" id="panel-email">
      <div class="card">
        ...
        <p class="sub">Receive one daily email after a successful digest update. The message includes 4–5 star papers with their reasons and full abstracts; other papers show titles, ratings, categories, authors, and arXiv links.</p>
        <label class="check-line" style="margin-top:16px">
          <input type="checkbox" id="enable_email" name="enable_email" onchange="toggleEmail()" {% if cur_email_enabled %}checked{% endif %}>
          Enable email notification
        </label>
...
<div class="header">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
    <h1 style="margin:0;font-size:20px"><img src="/static/brand-mark.svg" alt="" style="vertical-align:middle;margin-right:6px" width="20" height="20">Astro Digest</h1>
...
<script>
window.ASTRODIGEST_DIGEST_STATUS = {{ digest_status_map() | tojson }};
</script>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Astro Digest - {{ digest.date }}</title>
...
.reason-strong .reason-text b{color:#b45309}
.reason-high{background:#eefaf2;border-left-color:#27ae60}
.reason-high .reason-text b{color:#1e7e34}
...
{% for tier in digest.tiers %}
  ...
  <div class="card" id="card-{{ paper.paper_id | replace('.', '-') }}" data-paper-type="{{ paper.paper_type | default('new') }}" data-categories="{{ paper.categories }}" data-search-title="{{ paper.title }}" data-search-authors="{{ paper.authors }}" data-search-abstract="{{ full_abstracts.get(paper.paper_id, paper.abstract) }}" data-search-id="{{ paper.paper_id }}" data-search-categories="{{ paper.categories }}" data-base-score="{{ paper.base_score | default(paper.score) }}" data-score="{{ paper.score }}">
    ...
    {% if paper.score and not paper.scoring_failed and paper.score|int >= 4 %}
      ...
      {% endif %}
    {% endif %}
    {% if paper.reason and not paper.scoring_failed %}{% set reason_kind = 'strong' if paper.score|int == 5 else ('high' if paper.score|int == 4 else 'medium') %}<div class="card-reason reason-{{ reason_kind }}" data-role="reason" {% if paper.score|int < 4 %}hidden{% endif %}><span class="reason-text">{{ reason_html.get(paper.paper_id, '') | safe }}</span></div>{% endif %}
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Astro Digest - {{ selected_date }}</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f0f2f5;color:#333}
.header{height:74px;min-height:74px;background:#1a2332;color:#fff;padding:20px 32px}
...
def _load_full_abstracts(date_str, digest_path=None):
    ...
    and the 4–5 star email recommendations can use the complete text. Older
    digests without a sidecar degrade gracefully to the stored snippet.
    """
    if not date_str:
        return {}
    sidecar_path = (os.path.splitext(digest_path)[0] + '.full.json') if digest_path else os.path.join(
...
def _sweep_figure_retention() -> None:
    ...
    deleted. Cleanup waits until the backfill worker is idle because
    images are shared across digest dates.
    """
    try:
        with _figure_backfill_lock:
            # Files are shared across dates. Defer cleanup while any worker
            # owns the cache, including a digest without a sidecar yet.
            if _figure_backfill_state["running"]:
                return
            _sweep_idle_figure_retention()
    except Exception:
        pass  # retention is housekeeping; never disturb the caller


def _sweep_idle_figure_retention() -> None:
    """Called with the worker lock held so a backfill cannot start mid-sweep."""
    store = _load_figure_retention()
    ...
    for name in sidecar_names:
        match = re.search(r"^digest_(\d{4}-\d{2}-\d{2})(?:\.figures\.json|\.md)$", name)
        if not match:
            continue
        ...
        protected.update(pid for pid, score in effective.items() if score >= FIGURE_MIN_SCORE)
...
def post_feedback():
    ...
    cfg, _ = _load_config_and_env()
    term_source = dict(matched_paper)
    if 'technical_terms' not in term_source:
        # Pre-metadata digests keep their complete abstract in the existing
        # sidecar. Extract from it once; subsequent rebuilds use the snapshot.
        term_source['abstract'] = _load_full_abstracts(date_str, digest_path=digest_path).get(paper_id) or term_source.get('abstract', '')
    terms = feedback_technical_terms(term_source, known_terms=cfg.get('keywords', []))
    feedback.append({
            "paper_id": paper_id,
            ...
            "learning_candidates": [r for r in terms if r['learning_candidate']],
            "date": date_str,
...
def update_apply():
    ...
    with _update_lock:
        ...
        version = _update_state.get("latest") or ""
        zip_path = updater.UPDATES_DIR / f"AstroDigest-v{version}.zip"
    ...
    if getattr(sys, "frozen", False):
        updater_cmd = [str(Path(sys._MEIPASS) / "astrodigest-cli"), "--apply", str(marker_path)]
    else:
...
def _start_serving(server):
    ...
    global _server
    _server = server
    thread = Thread(target=server.serve_forever, name="astrodigest-http", daemon=True)
    thread.start()
    return thread
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Astro Digest desktop app")
    parser.add_argument(
        "--port",
        type=int,
        default=0,
    ...
    if not _acquire_single_instance_lock():
        ...
        if opened:
            print("Astro Digest is already running; focusing its window.")
        else:
            print("Astro Digest appears to be running, but its window could not be focused.")
        return
