...
from .technical_terms import normalize_technical_terms
...
def _paper_to_markdown_entry(paper: dict, brief: bool = False) -> list[str]:
    ...
    if 'technical_terms' in paper:
        # Metadata travels atomically with the existing Markdown file through
        # staging, copying, and reload. Older readers ignore the HTML comment.
        terms = normalize_technical_terms(paper['technical_terms'])
        payload = json.dumps({'domain': 'astronomy', 'subdomain': None, 'terms': terms},
                             ensure_ascii=False, allow_nan=False)
        payload = payload.replace('<', r'\u003c').replace('>', r'\u003e').replace('--', r'\u002d\u002d')
        lines.append(f'<!-- astrodigest-technical-terms:v1 {payload} -->')
