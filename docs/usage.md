...
Enable email in **Settings → Email Notification** and provide your SMTP settings. After a successful daily digest update, Astro Digest sends one email for that date. The message includes every 5-star paper with its title, score, reason, authors, categories, arXiv link, and full abstract; lower-rated papers remain available in the App. A message is still sent when there are no 5-star papers.

Daily sends are deduplicated by date. Use **Resend Latest Digest** in Settings or run `send_digests.py --resend YYYY-MM-DD` for an explicit resend. Messages include both HTML and plain-text parts and threading headers to help mailbox clients display the updates as one series. The App link uses the `astrodigest://` macOS URL scheme so it can launch the App and open the requested date when the App is not already running.

Old `astropaperdigest://` email links still open the matching digest in Astro Digest.

## Installation

### Option A: dmg installer (no Python required)

1. Download **`AstroDigest-<version>.dmg`** from GitHub Releases
2. Double-click it and drag **Astro Digest.app** into **Applications**
...
2. Double-click **`Astro Digest.app`** — a native desktop window opens automatically
...
When you save the profile setting, Astro Digest immediately reads a private copy in read-only mode and validates the library. If the default location cannot be read, the Settings page explains the error and asks for the real `zotero.sqlite` path. For custom data locations, check Zotero **Settings → Advanced → Files and Folders**.
...
- **Update source**: GitHub Releases (public repo). Check endpoint: `https://api.github.com/repos/jiangrz77/AstroDigest/releases/latest`.
...
Fresh packaged installations store user data in `~/Library/Application Support/AstroDigest`. If the previous `~/Library/Application Support/AstroPaperDigest` directory exists, Astro Digest keeps using it, preserving settings, paper history, feedback, and email delivery state. Source installations keep data beside `main.py`. Both old and new daily-digest titles can be read.

The macOS bundle identifier and legacy single-instance filenames stay stable across the rename. App updates can replace an installation under the old filename; downloading the new DMG provides `Astro Digest.app` directly. The executable inside the bundle remains `AstroDigest` for update and macOS identity compatibility.
